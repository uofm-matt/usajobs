"""Additional coverage for /api/filters — full response shape, filter params, SQL builder."""
...
class TestParameterization:
    ...
    def test_no_fstring_interpolation_of_user_values(self):
        source = inspect.getsource(_build_where)
        ...
        assert not (dangerous & fstring_vars)
...
class TestUnfilteredShape:
    ...
    async def test_cache_returns_same_payload(self, client):
        first = (await client.get("/api/filters")).json()
        second = (await client.get("/api/filters")).json()
        assert first == second
