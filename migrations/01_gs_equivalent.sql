-- Migration: Add GS-equivalent grade columns to jobs_geo
-- Maps all federal pay band systems to GS grade equivalents
--
-- Usage: psql -h localhost -U usajobs -d usajobs -f migrations/01_gs_equivalent.sql
...
BEGIN
    ...
    IF pay_plan IN ('GS','GL','GG','FG','SK','CT','CG','GP','GW',
                    'AD','SL','ES','TP','CY','CE','CC','IJ','BB',
                    'LP','CR','CP','TR','LE','NZ','HS','JS','PG',
                    'EL','NB','ST','IA','CS','MC','VN','VM','VH',
                    'VC','VP') THEN
    ...
    -- Catch-all: try to parse as integer
    IF grade ~ '^\d+$' THEN
    ...
    IF pay_plan IN ('WG','WL','WS','WB','WD','WK','WN','WY','WE',
        ...
        IF grade ~ '^\d+$' THEN
            RETURN CASE
                ...
                ELSE ARRAY[grade::integer, grade::integer]
            END;
        END IF;
        RETURN NULL;
    END IF;
